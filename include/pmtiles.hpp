...
const int8_t PMAP_OK = 0;
const int8_t PMAP_BAD_MAGIC = -1;
const int8_t PMAP_BAD_VERSION = -2;
const int8_t PMAP_VARINT_TOO_LONG = -3;
const int8_t PMAP_END_OF_BUFFER = -4;
const int8_t PMAP_TILE_ZOOM_OVERFLOW = -5;
const int8_t PMAP_TILE_XY_OUTSIDE_ZOOM = -6;
const int8_t PMAP_MALFORMED_DIRECTORY = -7;
...
inline headerv3 deserialize_header(const std::string_view &s,
                                   int8_t &pmerr = pmerrno) {
    if (s.substr(0, 7) != "PMTiles") {
        PMTILES_EXCEPTION(pmtiles_magic_number_exception{}, PMAP_BAD_MAGIC, headerv3());
    }
    if (s.size() != 127 || s[7] != 0x3) {
        PMTILES_EXCEPTION(pmtiles_version_exception{}, PMAP_BAD_VERSION, headerv3());
    }
    headerv3 h;
...
uint64_t decode_varint_impl(const char **data, const char *end,
    ...
    if (iend - begin >= max_varint_length) {  // fast path
        do {
            ...
            PMTILES_EXCEPTION(pmtiles_magic_number_exception{}, PMAP_VARINT_TOO_LONG, 0);
        } while (false);
    } else {
        ...
        if (p == iend) {
            PMTILES_EXCEPTION(end_of_buffer_exception{}, PMAP_END_OF_BUFFER, 0);
        }
...
inline zxy tileid_to_zxy(uint64_t tileid, int8_t &pmerr = pmerrno) {
    uint64_t acc = 0;
    ...
    PMTILES_EXCEPTION_OVF("tile zoom exceeds 64-bit limit", PMAP_TILE_ZOOM_OVERFLOW, zxy(0,0,0));
}

inline uint64_t zxy_to_tileid(uint8_t z, uint32_t x, uint32_t y, int8_t &pmerr = pmerrno) {
    if (z > 31) {
        PMTILES_EXCEPTION_OVF("tile zoom exceeds 64-bit limit", PMAP_TILE_ZOOM_OVERFLOW, 0);
    }
    if (x > (1U << z) - 1U || y > (1U << z) - 1U) {
        PMTILES_EXCEPTION_OVF("tile x/y outside zoom level bounds", PMAP_TILE_XY_OUTSIDE_ZOOM, 0);
    }
    uint64_t acc = 0;
...
inline std::vector<entryv3> deserialize_directory(const std::string_view decompressed, int8_t &pmerr = pmerrno) {
    ...
    if (num_entries_64bit / 4U > decompressed.size()) {
        PMTILES_EXCEPTION(malformed_directory_exception(), PMAP_MALFORMED_DIRECTORY, std::vector<entryv3>());
    }
...
inline std::pair<uint64_t, uint32_t> get_tile(const std::function<std::string(const std::string &, uint8_t)> decompress, const char *pmtiles_map, uint8_t z, uint32_t x, uint32_t y, int8_t &pmerr = pmerrno) {
    ...
    if (h.root_dir_bytes > std::numeric_limits<uint32_t>::max()) {
        PMTILES_EXCEPTION(malformed_directory_exception(), PMAP_MALFORMED_DIRECTORY, std::make_pair(0, 0));
    }
